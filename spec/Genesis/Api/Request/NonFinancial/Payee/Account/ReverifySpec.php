...
use Genesis\Api\Request\NonFinancial\Payee\Account\Reverify;
use Genesis\Builder;
use Genesis\Config;
...
class ReverifySpec extends ObjectBehavior
{
    use RequestExamples;
    ...
    {
        $this->shouldHaveType(Reverify::class);
    ...
    {
        $this->setRequestParameters();
        $this->getApiConfig('url')->shouldContain($this->payee_unique_id);
        $this->getApiConfig('url')->shouldContain($this->account_unique_id);
    }
    ...
    public function it_should_build_correct_url()
    {
        ...
        $this->getApiConfig('url')->shouldContain("payee/{$this->payee_unique_id}/account/{$this->account_unique_id}");
    }
    ...
    public function setRequestParameters()
    {
        ...
        $this->account_unique_id = $this->getFaker()->uuid;

        $this->setPayeeUniqueId($this->payee_unique_id);
        $this->setAccountUniqueId($this->account_unique_id);
    }
}
