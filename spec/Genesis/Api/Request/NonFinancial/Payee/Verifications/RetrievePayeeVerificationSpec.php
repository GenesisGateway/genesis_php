...
use Genesis\Api\Request\NonFinancial\Payee\Verifications\RetrievePayeeVerification;
use Genesis\Builder;
use Genesis\Config;
use PhpSpec\ObjectBehavior;
use spec\SharedExamples\Genesis\API\Request\RequestExamples;

class RetrievePayeeVerificationSpec extends ObjectBehavior
{
    use RequestExamples;

    protected $payeeUniqueId;

    protected $verificationUniqueId;

    public function it_is_initializable()
    {
        $this->shouldHaveType(RetrievePayeeVerification::class);
        $this->shouldBeAnInstanceOf(BaseRequest::class);
    }

    public function it_should_fail_when_missing_required_parameter()
    {
        $this->testMissingRequiredParameters(['payee_unique_id', 'verification_unique_id']);
    }
    ...
    public function it_should_build_correct_url()
    {
        ...
        $this->getApiConfig('url')->shouldBe(
            'https://staging.api.emerchantpay.net:443/payee/' . $this->payeeUniqueId . '/verifications/' . $this->verificationUniqueId
        );
    }
    ...
    public function setRequestParameters()
    {
        ...
        $this->verificationUniqueId = $this->getFaker()->uuid();
        $this->setPayeeUniqueId($this->payeeUniqueId);
        $this->setVerificationUniqueId($this->verificationUniqueId);
    }
}
